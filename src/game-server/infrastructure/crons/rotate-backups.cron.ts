...
export class RotateBackupsCron {
    ...
    @Cron('*/5 * * * *') // Every 2 hours at 5 minutes
    async handleCron(){
        ...
        this.logger.log('Finished running RotateBackupsCron', 'RotateBackupsCron');
    }
    ...
    async storeCurrentBackup() {
        ...
        for (const config of arkGameConfigs) {
            ...
            const backupsDir =
                basePath + '/' + serverConfig.backupsDestinationDirectory + '/' + config.mapFolderName;
            ...
            try {
                ...
                this.logger.log(
                    `Finished storing current backup for server ${
                        config.serverName
                    }`,
                    'RotateBackupsCron'
                );
                ...
                this.logger.error(
                    `Error storing current backup for server ${
                        config.serverName
                    }: ${e.message}`,
                    e.stack,
                    'RotateBackupsCron'
        ...
        this.logger.log('Finished running StoreCurrentBackupCron', 'StoreCurrentBackupCron');
    }
    ...
    async deleteOlderBackups(olderThanHours: number, subPath?: string) {
        ...
        for (const config of arkGameConfigs) {
            ...
            const serverConfig = arkGameServerConfigs[config.serverName];
            const backupsDir =
                basePath + '/' + serverConfig.backupsDestinationDirectory + '/' + config.mapFolderName + (subPath ? '/' + subPath : '');
            ...
            try {
                ...
                this.logger.log(
                    `Finished deleting older backups for server ${
                        config.serverName
                    }`,
                    'RotateBackupsCron'
                );
                ...
                this.logger.error(
                    `Error deleting older backups for server ${
                        config.serverName
                    }: ${e.message}`,
                    e.stack,
                    'RotateBackupsCron'
